<template>
    <shared-users-dialog
        ...
        :shared-users="users"
    />
    <q-item class="text-primary" v-ripple @click="openDialog" clickable>
...
<script lang="ts">
    ...
    import { sharedUsers } from 'src/firebase/shared-users'
    ...
    export default defineComponent({
        ...
        setup(props) {
            ...
            onMounted(async () => {
                users.value = await sharedUsers.getListUsers(props.listId)
            })
            ...
            async function addPerson(email: string) {
                if (await sharedUsers.addUser(props.listId, email))
                    notify({
